...
class DataStore {
    ...
    func createFreinds() {
        ...
        let oprahMessage2 = Message(text: "Nice to meet you...Nice to meet you...Nice to meet you...", date: Date().addingTimeInterval(-2*60), friend: oprah)
        let oprahMessage3 = Message(text: "how is everything going?how is everything going?how is everything going?how is everything going?how is everything going?", date: Date().addingTimeInterval(-1*60), friend: oprah)
        ...
        let sheldonMessage = Message(text: "Don't sit on my spot...!!!!", date: Date().addingTimeInterval(-24*10*60*60), friend: sheldon)
        ...
        try! realm.write {
            realm.add([oprah,lisa,ramsay,adele,sheldon,oprahMessage1,oprahMessage2,oprahMessage3,lisaMessage,ramsyMessage,adeleMessage,sheldonMessage])
        }
        ...
        let predict = NSPredicate(format: "friend.name = %@", friend.name)
           ...
           let fetcehedMessages = realm.objects(Message.self).sorted(byKeyPath: "date", ascending: false).filter(predict).first ?? Message(text: nil, date: nil, friend: friend)
